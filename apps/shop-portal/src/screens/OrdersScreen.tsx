...
// Singleton AudioContext — created on first user gesture to satisfy browser autoplay policy
...
type OrderStatus = 'pending' | 'confirmed' | 'assigned' | 'picked_up' | 'delivered' | 'cancelled'
...
const NEXT_ACTION: Partial<Record<OrderStatus, { label: string; next: string; danger?: boolean }>> = {
  pending:  { label: 'Confirm Order',        next: 'confirmed' },
  assigned: { label: 'Mark Ready for Pickup', next: 'picked_up' },
}
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: 'white', borderRadius: 14, padding: 24, width: 360,
        ...
        <div style={{ fontSize: 13, color: 'var(--gray-400)', marginBottom: 16 }}>
          Please select a reason — the customer will see this.
        </div>
        ...
        {selected === 'Other' && (
          <textarea
            ...
            style={{
              ...
              border: '1.5px solid var(--gray-200)', fontSize: 13, fontFamily: 'inherit',
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button
            ...
            disabled={!reason}
            style={{
              flex: 2, padding: '10px', borderRadius: 9, border: 'none', cursor: 'pointer',
    ...
    <div className="card" style={{ marginBottom: 12, padding: 0, overflow: 'hidden' }}>
      ...
      <div
        ...
        style={{
          ...
          cursor: 'pointer',
          background: order.status === 'pending' ? 'var(--yellow-light)' : 'white',
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <span style={{ fontWeight: 700, fontSize: 15 }}>{fmt(order.total_amount)}</span>
          <span style={{ color: 'var(--gray-400)', fontSize: 12 }}>{timeAgo(order.created_at)}</span>
          <span style={{ color: 'var(--gray-400)', fontSize: 12 }}>{expanded ? '▲' : '▼'}</span>
        </div>
      </div>
        ...
        <div style={{ padding: '14px 18px' }}>
          ...
          {order.items && order.items.length > 0 ? (
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--gray-400)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.4px' }}>
                Items
              </div>
              {order.items.map(item => (
                <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid var(--gray-100)' }}>
                  <div>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: 16 }}>
                    <span style={{ color: 'var(--gray-500)', fontSize: 12 }}>×{item.quantity}</span>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700, fontSize: 15, paddingTop: 8, borderTop: '1px solid var(--gray-200)', marginTop: 4 }}>
                <span>Total</span>
                <span>{fmt(order.total_amount)}</span>
              </div>
            </div>
          ) : (
            <div style={{ marginBottom: 14, color: 'var(--gray-400)', fontSize: 13 }}>
              Loading items…
            </div>
          )}
          ...
          <div style={{ fontSize: 12, color: 'var(--gray-500)', marginBottom: 14 }}>
            📍 <AddressDisplay raw={order.delivery_address as string} />
          </div>

          {/* Actions */}
          {(action || order.status === 'pending' || order.status === 'assigned') && (
            <div style={{ display: 'flex', gap: 10 }}>
              {action && (
                <button
...
export default function OrdersScreen() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      // Sound alert for new pending orders (skip on first load so we don't chime stale orders)
      const currentPendingIds = new Set(raw.filter(o => o.status === 'pending').map(o => o.id))
      if (!isFirstLoad.current) {
        const hasNewPending = [...currentPendingIds].some(id => !prevPendingIds.current.has(id))
        if (hasNewPending) playOrderChime()
      }
      isFirstLoad.current = false
      prevPendingIds.current = currentPendingIds

      // Enrich active orders with items
      const toEnrich = raw.filter(o => ['pending', 'confirmed', 'assigned'].includes(o.status))
  ...
  useEffect(() => {
    ...
    fetchOrders()
    pollRef.current = window.setInterval(fetchOrders, 8000)
    return () => {
  ...
  const handleAction = async (orderId: string, status: string) => {
    try {
      ...
      await fetchOrders()
      const label = status === 'confirmed' ? 'Order confirmed & rider assigned!'
        : status === 'picked_up' ? 'Marked ready — rider is heading to pick up!'
  ...
  const TabBtn = ({ id, label, count }: { id: Tab; label: string; count: number }) => (
    ...
    >
      ...
      {count > 0 && (
        <span style={{
          background: tab === id ? 'rgba(255,255,255,0.25)' : (id === 'pending' ? 'var(--yellow)' : 'var(--gray-200)'),
  ...
  return (
    <div style={{ maxWidth: 800 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--gray-400)' }}>Auto-refreshes every 8s</div>
      </div>
